...
def test_snoRNA(train_x,feature_id,test_x,gamma_list,clf):
    '''
    ...
    K_test = []
    for i in range(m):
        kk_test = kernel_RBF(test_x[:, feature_id[2 * i]:feature_id[2 * i + 1]],
                             train_x[:, feature_id[2 * i]:feature_id[2 * i + 1]], gamma_list[i])
    ...
    return y_pred, y_score, kernel_weights
